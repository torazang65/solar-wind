...
export OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/../dev/outputs/solar_cnn_v6_taeukjung}"
...
case "${ACTION}" in
  train)
    exec conda run --no-capture-output -n ASAI \
      python src_taeukjung/train_solar_cnn_v6.py
    ;;
  infer)
    exec conda run --no-capture-output -n ASAI \
      python src_taeukjung/inference_solar_cnn_v6.py
    ;;
  *)
