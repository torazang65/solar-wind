...
export OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/../dev/outputs/solar_probabilistic_taeukjung}"
...
case "${ACTION}" in
  train)
    exec conda run --no-capture-output -n ASAI \
      python src_taeukjung/train_solar_probabilistic.py
    ;;
  infer)
    exec conda run --no-capture-output -n ASAI \
      python src_taeukjung/inference_solar_probabilistic.py
    ;;
  *)
