...
from inference_solar_source_map_v11_2 import load_best_model, predict
...
def temporal_consistency_rows(prediction, actual, chain_ids, positions):
    prediction_error = []
    target_error = []
    pair_count = 0
    for chain_id in np.unique(chain_ids):
        rows = np.flatnonzero(chain_ids == chain_id)
        rows = rows[np.argsort(positions[rows])]
        for left, right in zip(rows[:-1], rows[1:]):
            if positions[right] != positions[left] + 1:
                continue
            prediction_error.append(prediction[left, 1:] - prediction[right, :-1])
            target_error.append(actual[left, 1:] - actual[right, :-1])
            pair_count += 1
    if not prediction_error:
        return {
            "pairs": 0,
            "prediction_overlap_rmse_km_s": float("nan"),
            "target_overlap_max_abs_km_s": float("nan"),
        }
    prediction_error = np.concatenate(prediction_error)
    target_error = np.concatenate(target_error)
    return {
        "pairs": pair_count,
        "prediction_overlap_rmse_km_s": float(
            np.sqrt(np.mean(prediction_error**2))
        ),
        "target_overlap_max_abs_km_s": float(np.max(np.abs(target_error))),
    }
...
def main():
    ...
    full_prediction, _, chain_ids, chain_positions, components = predict(
    ...
    consistency = temporal_consistency_rows(
        full_prediction, actual, chain_ids, chain_positions
    )
    consistency_path = OUTPUT_DIR / f"{FILE_STEM}_temporal_consistency.csv"
    pd.DataFrame([consistency]).to_csv(consistency_path, index=False)

    reconstruction_error = float(np.max(np.abs(full_prediction - reconstructed)))
    if reconstruction_error > 1e-3:
        raise RuntimeError(
            f"V11.2 component reconstruction failed: {reconstruction_error}"
        )
    if consistency["target_overlap_max_abs_km_s"] > 1e-3:
        raise RuntimeError(
            "validation targets do not have the expected one-step overlap"
        )
    ...
    print(
        ...
        f"prediction_overlap_rmse="
        f"{consistency['prediction_overlap_rmse_km_s']:.3f}"
    )
    ...
    print(f"saved: {consistency_path.resolve()}")
