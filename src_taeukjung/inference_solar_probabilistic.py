...
def load_best_model():
    ...
    if current_preprocess != checkpoint["preprocess"]:
        raise ValueError(
            "inference preprocessing does not match checkpoint: "
            f"current={current_preprocess}, checkpoint={checkpoint['preprocess']}"
        )

    model = SolarWindProbabilisticTransformer(**checkpoint["model_kwargs"]).to(DEVICE)
...
if __name__ == "__main__":
    ...
    baseline_rmse = np.sqrt(np.mean((linear_baseline - validation_target) ** 2, axis=0))
    validation_metrics["linear_baseline_rmse_km_s"] = baseline_rmse
    validation_metrics.to_csv(
        OUTPUT_DIR / "solar_probabilistic_validation_metrics.csv", index=False
    )
    ...
    submission_path = OUTPUT_DIR / "solar_probabilistic_submission.csv"
    submission.to_csv(submission_path, index=False)

    uncertainty = pd.DataFrame(test_std, columns=TARGET_COLUMNS)
    uncertainty.insert(0, "sample_id", predicted_ids)
    uncertainty_path = OUTPUT_DIR / "solar_probabilistic_test_std.csv"
