...
from train_solar_hybrid_v10 import (
...
FEATURE_SCHEMA = (
    "seokho_v7_cell_source_map_masked_intensity_signed_delta4_"
    "ar2_backmapping_bounded_propagation_v11"
)


def fit_ar_configuration(train_chains, val_chains):
    order = int(os.getenv("V11_AR_ORDER", "2"))
    ridge_strength = float(os.getenv("V11_AR_RIDGE", "30"))
    fit = fit_global_ar(
        train_inputs,
        train_targets,
        train_chains,
...
def build_model_kwargs(ar_fit, ar_residual_scale, wind_only=False):
    return {
        ...
        "source_hidden_dim": int(os.getenv("V11_SOURCE_HIDDEN_DIM", "64")),
        "dropout": float(os.getenv("V11_DROPOUT", "0.10")),
        "time_mask_prob": float(os.getenv("V11_TIME_MASK_PROBABILITY", "0.05")),
        "modality_drop_prob": float(
            os.getenv("V11_MODALITY_DROP_PROBABILITY", "0.15")
        ),
        "propagation_cap_multiplier": float(
            os.getenv("V11_PROPAGATION_CAP_MULTIPLIER", "1.25")
        ),
        "fixed_lag_hours": float(os.getenv("V11_FIXED_LAG_HOURS", "96")),
        "fixed_lag_reference_speed_kms": float(
            os.getenv("V11_FIXED_LAG_REFERENCE_SPEED_KMS", "430")
        ),
        "delta_gain": float(os.getenv("V11_DELTA_GAIN", "4.0")),
        ...
        "solar_disk_edge_pixels": float(
            os.getenv("V11_SOLAR_DISK_EDGE_PIXELS", "1.5")
        ),
        "climatology_speed_kms": float(
            os.getenv("V11_CLIMATOLOGY_SPEED_KMS", "430")
        ),
        "kernel_sigma_hours": float(
            os.getenv("V11_KERNEL_SIGMA_HOURS", "12")
        ),
        "fallback_weight": float(os.getenv("V11_FALLBACK_WEIGHT", "1.0")),
        "transit_residual_hours": float(
            os.getenv("V11_TRANSIT_RESIDUAL_HOURS", "18")
        ),
        "fast_wind_threshold_kms": float(
            os.getenv("V11_FAST_WIND_THRESHOLD_KMS", "550")
        ),
        "fast_wind_scale_kms": float(
            os.getenv("V11_FAST_WIND_SCALE_KMS", "50")
        ),
        "fast_quiet_suppression": float(
            os.getenv("V11_FAST_QUIET_SUPPRESSION", "0.50")
        ),
    }
...
def is_physical_parameter(name):
    return name in {
        ...
        "fusion_gate_head.bias",
    }


def make_optimizer(model):
    peak_lr = float(os.getenv("LEARNING_RATE", "5e-5"))
    weight_decay = float(os.getenv("V11_WEIGHT_DECAY", "0.02"))
    physical_multiplier = float(os.getenv("V11_PHYSICAL_LR_MULT", "10"))
    base_parameters = []
    physical_parameters = []
...
def make_scheduler(optimizer, peak_lr):
    warmup_epochs = int(os.getenv("V11_WARMUP_EPOCHS", "2"))
    minimum_lr = float(os.getenv("V11_MIN_LR", "1e-6"))
    decay_epochs = int(os.getenv("V11_COSINE_DECAY_EPOCHS", str(EPOCHS)))
    if not 0 < warmup_epochs < EPOCHS:
        raise ValueError("V11_WARMUP_EPOCHS must be between 1 and EPOCHS - 1")
    if not warmup_epochs < decay_epochs <= EPOCHS:
        raise ValueError(
            "V11_COSINE_DECAY_EPOCHS must be after warmup and no greater than EPOCHS"
        )
    ...
    return scheduler, warmup_epochs, minimum_lr, decay_epochs


def alignment_kl(model, aux, wind, target, sigma_deg):
    """Align source-map kernel mass with label-derived backmapping paths."""
    ...
    with torch.autocast(AMP_DEVICE_TYPE, enabled=False):
        ...
        transit_hours = model.dist_eff_hours_speed.float() / y_series.clamp(0.2, 1.2)
        delta_time = (
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        with torch.set_grad_enabled(training):
            with torch.amp.autocast(AMP_DEVICE_TYPE, enabled=USE_AMP):
                ...
                surge_label = (
                    ...
                    > float(os.getenv("V11_SURGE_THRESHOLD_KMS", "100")) / 1000.0
                ).float().unsqueeze(-1)
                ...
                if component_l2_weight > 0.0:
                    scale = model.baseline_residual_scale.to(
                        dtype=prediction.dtype
                    )
                    standardized_energy = (
                        components["propagation_residual"] / scale
                    ).square().mean()
        ...
        if batch_index == 1 or batch_index % 20 == 0 or batch_index == len(loader):
            ...
            print(
                ...
                f"ar_rmse={math.sqrt(totals['ar'] / counts['forecast']):.3f} "
...
def main():
    ...
    model = SolarWindSourceMapV11(**model_kwargs).to(DEVICE)
    optimizer, peak_lr, weight_decay, physical_multiplier = make_optimizer(model)
    scheduler, warmup_epochs, minimum_lr, decay_epochs = make_scheduler(
        optimizer, peak_lr
    )
    scaler = torch.amp.GradScaler(AMP_DEVICE_TYPE, enabled=USE_AMP)
    ema_decay = float(os.getenv("V11_EMA_DECAY", "0.995"))
    ema = ExponentialMovingAverage(model, ema_decay) if ema_decay > 0.0 else None

    hindcast_start = float(os.getenv("V11_HINDCAST_WEIGHT_START", "0.70"))
    hindcast_end = float(os.getenv("V11_HINDCAST_WEIGHT_END", "0.10"))
    hindcast_decay_epochs = int(os.getenv("V11_HINDCAST_DECAY_EPOCHS", "8"))
    transit_l2_weight = float(os.getenv("V11_TRANSIT_RESIDUAL_L2", "0.003"))
    alignment_weight = float(os.getenv("V11_ALIGNMENT_WEIGHT", "0.02"))
    alignment_sigma_deg = float(os.getenv("V11_ALIGNMENT_SIGMA_DEG", "20"))
    surge_weight = float(os.getenv("V11_SURGE_WEIGHT", "0.02"))
    surge_pos_weight = torch.tensor(
        float(os.getenv("V11_SURGE_POS_WEIGHT", "2.3")), device=DEVICE
    )
    component_l2_weight = float(os.getenv("V11_COMPONENT_L2", "0.001"))
    patience = int(os.getenv("V11_EARLY_STOP_PATIENCE", "10"))
    ...
    print(
        ...
        f"fixed_lag_h={model.fixed_lag_hours:.1f} mask={SOLAR_DISK_MASK} "
        f"norm={IMAGE_NORM} loss=rmse+hindcast+backmapping"
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            ...
            f"ar_val_rmse={val_metrics['ar_rmse']:.3f} "
            ...
            torch.save(
                {
                    ...
                    "preprocess": {
                        ...
                        "cosine_decay_epochs": decay_epochs,
