...
def main():
    model, checkpoint = load_best_model(
        SolarWindSourceMapV11, ARCHITECTURE_NAME, FILE_STEM
    )
    ...
    propagation_prediction = ar_prediction + components["propagation_residual"]
    variants = {
        "ar_only": ar_prediction,
        "source_only": source_prediction,
        "ar_plus_bounded_source": propagation_prediction,
        "full_v11": full_prediction,
    }
    ...
    groups = {
        ...
        "fast_quiet": (latest_wind >= 550.0) & (future_change <= 100.0),
    }
    rows = []
    ...
    reconstruction_error = float(
        np.max(np.abs(full_prediction - propagation_prediction))
    )
    if reconstruction_error > 1e-3:
        raise RuntimeError(
            f"V11 component reconstruction failed: {reconstruction_error}"
        )
