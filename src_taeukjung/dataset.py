...
def normalize_images_uint8(images):
    x = np.asarray(images, dtype=np.float32) / 255.0
    if IMAGE_NORM == "linear":
        return x
    pure_cubic = 0.5 * ((2.0 * x - 1.0) ** 3 + 1.0)
    if IMAGE_NORM == "cubic":
        return pure_cubic
    if IMAGE_NORM == "soft_cubic":
        return (1.0 - SOFT_CUBIC_STRENGTH) * x + SOFT_CUBIC_STRENGTH * pure_cubic
    raise ValueError(f"unknown IMAGE_NORM: {IMAGE_NORM}")
            ...
            for channel_index, channel in enumerate(CHANNELS):
                with Image.open(image_root / channel / filename) as image:
                    ...
                    if grayscale.size != (IMAGE_SIZE, IMAGE_SIZE):
                        grayscale = grayscale.resize((IMAGE_SIZE, IMAGE_SIZE), resampling)
                    resized_images[index, channel_index] = np.asarray(grayscale, dtype=np.uint8)
...
class SolarWindDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        images = normalize_images_uint8(self.image_array[self.image_indexes[row_index]])
        result = {
