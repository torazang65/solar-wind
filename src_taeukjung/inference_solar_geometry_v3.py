...
def load_best_model():
    ...
    if checkpoint.get("architecture") != "SolarWindGeometryTransformerV3":
        raise ValueError(f"not a v3 checkpoint: {checkpoint.get('architecture')}")
    ...
    model = SolarWindGeometryTransformerV3(**checkpoint["model_kwargs"]).to(DEVICE)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
    print(
        f"loaded v3 best epoch={checkpoint['epoch']} "
        ...
        f"mask_radius={checkpoint_preprocess['solar_disk_radius_fraction']:.3f} "
        f"cea_radius={checkpoint_preprocess['solar_cea_radius_fraction']:.3f}"
    )
    return model
...
if __name__ == "__main__":
    ...
    print("\n[Running V3 Validation Evaluation]")
    validation_prediction, _ = predict(model, val_loader)
    ...
    validation_metrics["linear_baseline_rmse_km_s"] = np.sqrt(
        np.mean((linear_baseline - validation_target) ** 2, axis=0)
    )
    validation_metrics.to_csv(
        OUTPUT_DIR / "solar_geometry_v3_validation_metrics.csv", index=False
    )
    ...
    submission_path = OUTPUT_DIR / "solar_geometry_v3_submission.csv"
