...
from model_solar_lstm_unet_v12_1 import FILE_STEM
...
def main():
    root = Path(
        os.getenv(
            "V12_1_ABLATION_ROOT",
            "/home/jovyan/outputs/solar_lag_lstm_unet_v12_1_seed777",
        )
    )
    rows = []
    ...
    if not rows:
        raise FileNotFoundError(f"no V12.1 histories found below {root}")
