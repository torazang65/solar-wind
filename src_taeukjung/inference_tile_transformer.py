...
def load_best_model():
    ...
    if current_preprocess != expected_preprocess:
        raise ValueError(
            "inference preprocessing does not match checkpoint: "
            f"current={current_preprocess}, checkpoint={expected_preprocess}"
        )

    model = SolarWindTileTransformer(**checkpoint["model_kwargs"]).to(DEVICE)
...
if __name__ == "__main__":
    ...
    baseline_metrics = metrics_by_horizon(validation_target, linear_baseline)
    validation_metrics["linear_baseline_rmse_km_s"] = baseline_metrics.rmse_km_s
    validation_metrics.to_csv(
        OUTPUT_DIR / "tile_transformer_validation_metrics.csv", index=False
    )
    ...
    submission_path = OUTPUT_DIR / "tile_transformer_submission.csv"
