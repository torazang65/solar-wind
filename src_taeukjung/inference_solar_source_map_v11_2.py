...
from train_solar_source_map_v11_2 import FEATURE_SCHEMA


def _expected_source_grid():
    return [
        20,
        int(os.getenv("V112_GRID_ROWS", "2")),
        int(os.getenv("V112_GRID_COLUMNS", "4")),
    ]
...
def load_best_model():
    ...
    if checkpoint.get("version") != "11.2":
    ...
    current_preprocess = {
        ...
        "source_grid": _expected_source_grid(),
    }
    if (
    ...
    for key, current in current_preprocess.items():
        expected = checkpoint_preprocess[key]
        if current != expected:
            raise ValueError(
    ...
    if model_kwargs["grid_rows"] != current_preprocess["source_grid"][1]:
        raise ValueError("checkpoint grid_rows disagrees with preprocessing")
    if model_kwargs["grid_columns"] != current_preprocess["source_grid"][2]:
        raise ValueError("checkpoint grid_columns disagrees with preprocessing")
    ...
    print(
        ...
        f"dist_eff_h={float(model.effective_distance().detach().cpu()):.2f}"
    )
...
def predict(model, loader, return_components=False):
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        for name, value in components.items():
            scale = (
                1.0
                if name in {"fusion_alpha", "surge_probability"}
                else 1000.0
            )
    ...
    components = {
        name: np.concatenate(values) for name, values in component_values.items()
    }
    return (
...
def main():
    ...
    val_prediction, val_ids, chain_ids, chain_positions, _ = predict(
        model, val_loader
    )
